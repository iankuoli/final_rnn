...
train :
  ...
  ha: Theano variable or None
    ...
    self.function_Gv = theano.function(inputs + v + [coefficient], Gv, givens=givens, on_unused_input='ignore')
  ...
  save_progress: string or None
    ...
    if isinstance(save_progress, str) and os.path.isfile(save_progress):
      save = pickle.load(open(save_progress))
    ...
    try:
      for u in range(first_iteration, 1 + num_updates):
        ...
        if isinstance(save_progress, str):
          ...
          pickle.dump(save, open(save_progress, 'wb'), pickle.HIGHEST_PROTOCOL)
